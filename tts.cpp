...
// 락을 밖에서 잡고 호출하는 안전한 형태로 변경
void stop_current_voice_internal() {
    if (vm.current_pid > 0) {
        kill(vm.current_pid, SIGKILL); // SIGTERM보다 확실한 강제 종료를 위해 SIGKILL 사용 가능
        int status;
        waitpid(vm.current_pid, &status, 0); // 좀비 프로세스 방지
...
void speak(const char* text, int priority, double debounce_time) {
    ...
    if (priority == 1) {
        stop_current_voice_internal(); // 내부 잠금 버전 호출로 안전성 확보
        queue_clear(&vm);              // 고쳐진 매개변수 (&vm.lock -> &vm)
    }
...
int main() {
    ...
    speak("첫 번째 안내 방송입니다.", 3, 2.0);
    sleep(1);
    
    printf("--> 긴급 문장 투입!\n");
    speak("화재가 발생했습니다. 대피하세요!", 1, 2.0);
